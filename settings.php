...
define('R4MP_NETWORK_ACTIVE', is_multisite() && is_plugin_active_for_network(plugin_basename(__FILE__)));

function r4mp_get_setting($key, $default = false) {
    return R4MP_NETWORK_ACTIVE ? get_site_option($key, $default) : get_option($key, $default);
}

function r4mp_save_setting($key, $value) {
    return R4MP_NETWORK_ACTIVE ? update_site_option($key, $value) : update_option($key, $value);
}
...
add_action('admin_menu', function() {
    ...
    add_submenu_page('r4mp-settings', 'Settings', 'Settings', 'manage_options', 'r4mp-settings', 'r4mp_settings_page');
    add_submenu_page('r4mp-settings', 'Missing Image', 'Missing Image', 'manage_options', 'r4mp-image-settings', 'r4mp_image_settings_page');
...
add_action('admin_init', function() {
    register_setting('r4mp_settings_group', 'r4mp_enabled', ['type' => 'boolean']);
    register_setting('r4mp_settings_group', 'r4mp_post_types', ['type' => 'array']);
    register_setting('r4mp_image_group', 'r4mp_default_image_url', ['sanitize_callback' => 'esc_url']);
    register_setting('r4mp_404_group', 'r4mp_custom_404_url', ['sanitize_callback' => 'esc_url']);
        ...
        <form method="post" action="options.php">
            ...
            <table class="form-table">
                <tr><th>Enable Redirect Matching</th>
                    <td><input type="checkbox" name="r4mp_enabled" value="1" <?php checked(1, r4mp_get_setting('r4mp_enabled', 1)); ?> /></td></tr>
                <tr><th>Post Types to Match</th>
                    <td>
                        <?php
        ...
        <form method="post" action="options.php">
            ...
            <table class="form-table">
                <tr><th>Default Image URL</th>
                    <td><input type="text" name="r4mp_default_image_url" value="<?php echo esc_attr(r4mp_get_setting('r4mp_default_image_url', '')); ?>" style="width: 400px;" />
        ...
        <form method="post" action="options.php">
            ...
            <table class="form-table">
                <tr><th>Custom 404 Page URL</th>
                    <td><input type="text" name="r4mp_custom_404_url" value="<?php echo esc_attr(r4mp_get_setting('r4mp_custom_404_url', '')); ?>" style="width: 400px;" />
        ...
        <form method="post" action="options.php">
            ...
            <table class="form-table">
                <tr><th>License Key</th>
                    <td><input type="text" name="r4mp_license_key" value="<?php echo esc_attr(r4mp_get_setting('r4mp_license_key', '')); ?>" style="width: 400px;" />
...
if (r4mp_is_pro_active()) {
    add_action('wp_head', function() {
        $default_img = esc_url(r4mp_get_setting('r4mp_default_image_url', ''));
        if ($default_img) {
            echo "<script>
